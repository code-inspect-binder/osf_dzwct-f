...
##TTS_HP2/TTS_HP3
TTS_HP2<-read.csv(file="TTS_HP2.csv", sep = "", header=FALSE)
colnames(TTS_HP2)<-c(colheaderChronos)
DW_TTS_HP2=add_column(TTS_HP2, GR, .before = 1)
DL_TTS_HP2=melt(DW_TTS_HP2, id=c("GR"), measured=c("colheaderChronos"))
colnames(DL_TTS_HP2)<-c("GrNr", "TrialNr","TTS_HP2")

TTS_HP3<-read.csv(file="TTS_HP3.csv", sep = "", header=FALSE)
colnames(TTS_HP3)<-c(colheaderChronos)
DW_TTS_HP3=add_column(TTS_HP3, GR, .before = 1)
DL_TTS_HP3=melt(DW_TTS_HP3, id=c("GR"), measured=c("colheaderChronos"))
colnames(DL_TTS_HP3)<-c("GrNr", "TrialNr","TTS_HP3")
TIS_Hdummy= merge(DL_TTS_HP2,DL_TTS_HP3, by=c("GrNr", "TrialNr"))
...
myData$TTS_HP2[myData$TTS_HP2==Inf]="15"
myData$TTS_HP3[myData$TTS_HP3==Inf]="20"
myData$TTS_HP2<-as.numeric(myData$TTS_HP2)
myData$TTS_HP3<-as.numeric(myData$TTS_HP3)
...
dat3=(no_outliers3)
DVoutP3=dat3$TTS_HP3
dim(dat3)

##LMER
options(contrasts = c("contr.sum","contr.poly"))

m_TTS_HP2 <- lmer(DVoutP2 ~ Count_neg_group + Count_pos_group + (1 | GrNr), data=dat2, REML=FALSE)
m_TTS_HP3 <- lmer(DVoutP3 ~ Count_neg_group + Count_pos_group + (1 | GrNr), data=dat3, REML=FALSE)

 
tab_model(m_TTS_HP2)
tab_model(m_TTS_HP3)

by(dat2$TTS_HP2, dat2$Count_neg_group, stat.desc, basic = FALSE, norm = TRUE)

plot_model(m_TTS_HP2, type="eff", terms=c("Count_pos_group","Count_neg_group"))

effect_plot( m_TTS_HP2, pred = Count_neg_group, interval = TRUE)+theme(text = element_text(size = 30))
