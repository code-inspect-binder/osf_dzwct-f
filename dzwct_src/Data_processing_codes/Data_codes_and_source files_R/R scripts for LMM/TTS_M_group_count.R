...
##TTS_MP2/TTS_MP3
TTS_MP2<-read.csv(file="TTS_MP2.csv", sep = "", header=FALSE)
colnames(TTS_MP2)<-c(colheaderChronos)
DW_TTS_MP2=add_column(TTS_MP2, GR, .before = 1)
DL_TTS_MP2=melt(DW_TTS_MP2, id=c("GR"), measured=c("colheaderChronos"))
colnames(DL_TTS_MP2)<-c("GrNr", "TrialNr","TTS_MP2")

TTS_MP3<-read.csv(file="TTS_MP3.csv", sep = "", header=FALSE)
colnames(TTS_MP3)<-c(colheaderChronos)
DW_TTS_MP3=add_column(TTS_MP3, GR, .before = 1)
DL_TTS_MP3=melt(DW_TTS_MP3, id=c("GR"), measured=c("colheaderChronos"))
colnames(DL_TTS_MP3)<-c("GrNr", "TrialNr","TTS_MP3")
TIS_Hdummy= merge(DL_TTS_MP2,DL_TTS_MP3, by=c("GrNr", "TrialNr"))
...
myData$TTS_MP2[myData$TTS_MP2==Inf]="15"
myData$TTS_MP3[myData$TTS_MP3==Inf]="20"
myData$TTS_MP2<-as.numeric(myData$TTS_MP2)
myData$TTS_MP3<-as.numeric(myData$TTS_MP3)
...
dat3=(no_outliers3)
DVoutP3=dat3$TTS_MP3
dim(dat3)

##LMER
options(contrasts = c("contr.sum","contr.poly"))

m_TTS_MP2 <- lmer(DVoutP2 ~ Count_neg_group + Count_pos_group + (1 | GrNr), data=dat2, REML=FALSE)
m_TTS_MP3 <- lmer(DVoutP3 ~ Count_neg_group + Count_pos_group + (1 | GrNr), data=dat3, REML=FALSE)
