...
##NCY_P2/NCYP3
NCYP2<-read.csv(file="NCYP2.csv", sep = "", header=FALSE)
colnames(NCYP2)<-c(colheaderChronos)
DW_NCYP2=add_column(NCYP2, GR, .before = 1)
DL_NCYP2=melt(DW_NCYP2, id=c("GR"), measured=c("colheaderChronos"))
colnames(DL_NCYP2)<-c("GrNr", "TrialNr","NCYP2")

NCYP3<-read.csv(file="NCYP3.csv", sep = "", header=FALSE)
colnames(NCYP3)<-c(colheaderChronos)
DW_NCYP3=add_column(NCYP3, GR, .before = 1)
DL_NCYP3=melt(DW_NCYP3, id=c("GR"), measured=c("colheaderChronos"))
colnames(DL_NCYP3)<-c("GrNr", "TrialNr","NCYP3")
NCYdummy= merge(DL_NCYP2,DL_NCYP3, by=c("GrNr", "TrialNr"))
...
dummy=merge(NCYdummy,EMO_count, by='TrialNr')

dummy$TrialNr=sapply(dummy,function(x) ifelse(x !="",paste("T",x),x))[,3]
...
m_NCYP2 <- lmer(DVoutP2 ~Count_neg_group + Count_pos_group + (1 | GrNr), data=dat2, REML=FALSE)
m_NCYP3 <- lmer(DVoutP3 ~Count_neg_group + Count_pos_group + (1 | GrNr), data=dat3, REML=FALSE)



tab_model(m_NCYP2)
tab_model(m_NCYP3)
by(dat2$NCYP2, dat2$Count_neg_group, stat.desc, basic = FALSE, norm = TRUE)
by(dat2$NCYP2, dat2$Count_pos_group, stat.desc, basic = FALSE, norm = TRUE)
by(dat3$NCYP3, dat3$Count_neg_group, stat.desc, basic = FALSE, norm = TRUE)
by(dat3$NCYP3, dat3$Count_pos_group, stat.desc, basic = FALSE, norm = TRUE)

plot_model(m_NCYP2, type="eff", terms=c("Count_pos_group","Count_neg_group"))
