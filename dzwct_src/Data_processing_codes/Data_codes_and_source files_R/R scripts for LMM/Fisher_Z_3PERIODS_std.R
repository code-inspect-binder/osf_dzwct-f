...
##P2 processing
OPAstdP2<-read.csv(file="OPAstdP2.csv", sep = "", header=FALSE)
colnames(OPAstdP2)<-c(colheaderChronos)
DW_OPAstdP2=add_column(OPAstdP2, GR, .before = 1)
FOPAstdP2=FisherZ(OPAstdP2)
DW_FOPAstdP2=add_column(FOPAstdP2, GR, .before = 1)
DL_OPAstdP2=melt(DW_OPAstdP2, id=c("GR"), measured=c("colheaderChronos"))
DL_FOPAstdP2=melt(DW_FOPAstdP2, id=c("GR"), measured=c("colheaderChronos"))
colnames(DL_OPAstdP2)<-c("GrNr", "TrialNr","OPAstdP2")
colnames(DL_FOPAstdP2)<-c("GrNr", "TrialNr","FOPAstdP2")

##P3 processing
OPAstdP3<-read.csv(file="OPAstdP3.csv", sep = "", header=FALSE)
colnames(OPAstdP3)<-c(colheaderChronos)
DW_OPAstdP3=add_column(OPAstdP3, GR, .before = 1)
FOPAstdP3=FisherZ(OPAstdP3)
DW_FOPAstdP3=add_column(FOPAstdP3, GR, .before = 1)
DL_OPAstdP3=melt(DW_OPAstdP3, id=c("GR"), measured=c("colheaderChronos"))
DL_FOPAstdP3=melt(DW_FOPAstdP3, id=c("GR"), measured=c("colheaderChronos"))
colnames(DL_OPAstdP3)<-c("GrNr", "TrialNr","OPAstdP3")
colnames(DL_FOPAstdP3)<-c("GrNr", "TrialNr","FOPAstdP3")
...
dummyproto1=merge(DL_OPAstdP1, DL_FOPAstdP1, by=c('TrialNr','GrNr'))
dummyproto2=merge( DL_OPAstdP2, DL_FOPAstdP2, by=c('TrialNr','GrNr'))
dummyproto3=merge( DL_OPAstdP3, DL_FOPAstdP3, by=c('TrialNr','GrNr'))
...
STAT_DL_OPAstdP123_EMO=myData[complete.cases(myData), ]


#histogram
histogramOPA<-ggplot(STAT_DL_OPAstdP123_EMO, aes(OPAstdP1)) + theme(legend.position = "none") + geom_histogram(aes(y = ..density..), colour = "black", fill = "white") + labs(x = "OPAstdP1", y = "Density") 
histogramOPA
histogramFOPA<-ggplot(STAT_DL_OPAstdP123_EMO, aes(FOPAstdP1)) + theme(legend.position = "none") + geom_histogram(aes(y = ..density..), colour = "black", fill = "white") + labs(x = "FOPAstdP1", y = "Density") 
histogramFOPA

STAT_DL_OPAstdP123_EMO$GrNr<-as.factor(STAT_DL_OPAstdP123_EMO$GrNr)
STAT_DL_OPAstdP123_EMO$GrNr<-factor(STAT_DL_OPAstdP123_EMO$GrNr, levels=c("G1","G2","G3","G4","G5","G6","G7","G8","G9","G10","G11","G12","G13","G14","G15"))
STAT_DL_OPAstdP123_EMO$count_neg_group<-as.factor(STAT_DL_OPAstdP123_EMO$Nbm1)
STAT_DL_OPAstdP123_EMO$count_neg_group<-factor(STAT_DL_OPAstdP123_EMO$Nbm1, levels=c("0","1","2","3"))
STAT_DL_OPAstdP123_EMO$count_pos_group<-as.factor(STAT_DL_OPAstdP123_EMO$Nbp1)
STAT_DL_OPAstdP123_EMO$count_pos_group<-factor(STAT_DL_OPAstdP123_EMO$Nbp1, levels=c("0","1","2","3"))

#outlier removal
df=STAT_DL_OPAstdP123_EMO
DV2=df$FOPAstdP2
DV3=df$FOPAstdP3
...
tab_model(m_FOPA_P2)
tab_model(m_FOPA_P3)
by(dat2$FOPAstdP2, dat2$count_neg_group, stat.desc, basic = FALSE, norm = TRUE)
by(dat3$FOPAstdP3, dat3$count_pos_group, stat.desc, basic = FALSE, norm = TRUE)
