...
##TIS_WP2/TIS_WP3
TIS_WP2<-read.csv(file="TIS_WP2.csv", sep = "", header=FALSE)
colnames(TIS_WP2)<-c(colheaderChronos)
DW_TIS_WP2=add_column(TIS_WP2, GR, .before = 1)
DL_TIS_WP2=melt(DW_TIS_WP2, id=c("GR"), measured=c("colheaderChronos"))
colnames(DL_TIS_WP2)<-c("GrNr", "TrialNr","TIS_WP2")

TIS_WP3<-read.csv(file="TIS_WP3.csv", sep = "", header=FALSE)
colnames(TIS_WP3)<-c(colheaderChronos)
DW_TIS_WP3=add_column(TIS_WP3, GR, .before = 1)
DL_TIS_WP3=melt(DW_TIS_WP3, id=c("GR"), measured=c("colheaderChronos"))
colnames(DL_TIS_WP3)<-c("GrNr", "TrialNr","TIS_WP3")
TIS_Wdummy= merge(DL_TIS_WP2,DL_TIS_WP3, by=c("GrNr", "TrialNr"))
...
dummy=merge(TIS_Wdummy,EMO_count, by='TrialNr')
...
dat3=(no_outliers3)
DVoutP3=dat3$TIS_WP3
##LMER
options(contrasts = c("contr.sum","contr.poly"))

m_TIS_WP2 <- lmer(DVoutP2 ~ Count_neg_group + Count_pos_group + (1 | GrNr), data=dat2, REML=FALSE)
m_TIS_WP3 <- lmer(DVoutP3 ~ Count_neg_group + Count_pos_group + (1 | GrNr), data=dat3, REML=FALSE)

tab_model(m_TIS_WP2)
tab_model(m_TIS_WP3)

by(dat2$TIS_WP2, dat2$Count_neg_group, stat.desc, basic = FALSE, norm = TRUE)
by(dat3$TIS_WP3, dat3$Count_pos_group, stat.desc, basic = FALSE, norm = TRUE)
plot_model(m_TIS_WP2, type="eff", terms=c("Count_pos_group","Count_neg_group"))

effect_plot( m_TIS_WP2, pred = Count_neg_group, interval = TRUE)+theme(text = element_text(size = 30))
